...
function ScoreBadge({ score, recommendation }: { score: number | null; recommendation: string | null }) {
  ...
  const color =
    score >= 70 ? 'bg-emerald/10 text-emerald border-emerald/20' :
    score >= 50 ? 'bg-amber/10 text-amber border-amber/20' :
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-text-secondary -mt-4">
        Upload a saved LinkedIn feed HTML file. We'll extract posts, detect job-related keywords, filter for programmer jobs, and score the fit against your resume.
      </p>

      {/* Filters */}
      {showFilters && (
        <div className="glass-card rounded-xl p-4 space-y-4 animate-fade-in-up">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <span className="text-sm text-text-secondary">Programmer jobs only</span>
            </label>
            ...
            {/* Min score */}
            <div className="flex items-center gap-2">
              <span className="text-xs text-text-muted shrink-0">Min score</span>
              <input
                type="number"
                min={0}
                max={100}
      ...
      {isProcessing && batch.data && (
        <div className="glass-card rounded-xl p-4 space-y-3 animate-fade-in-up">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-mono text-cyan">{batchProgress}%</span>
          </div>
              ...
              <div className="p-4 sm:p-5 space-y-3">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    {post.authorHeadline && (
                      <p className="text-xs text-text-muted mt-0.5">{post.authorHeadline}</p>
                    )}
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    {post.appStatus && post.appStatus !== 'not_applied' && (
                      <span className={cn('inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium', APP_STATUS_COLOR[post.appStatus as keyof typeof APP_STATUS_COLOR] ?? 'badge-slate')}>
                        {APP_STATUS_LABEL[post.appStatus as keyof typeof APP_STATUS_LABEL]}
                      </span>
                    )}
                ...
                {post.aiAnalyzed && (
                  <div className="pt-3 border-t border-border-subtle space-y-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <ScoreBadge score={post.score} recommendation={post.recommendation} />
                    ...
                    {post.summary && (
                      <p className="text-xs text-text-secondary">{post.summary}</p>
                    )}
